...
log = logging.getLogger('journaltools')
...
class ConfigSection(collections.MutableMapping):
    ...
    @property
    def root(self):
        """Returns the root :class:`~journaltools.ConfigSection` object. Read-only."""
        return self._root
    ...
    def init(self, key, default, type=None, comment=None):
        ...
        file in a manner consistent with the active :class:`~journaltools.Format`.
        """
    ...
    def section(self, key, create=None):
        ...
        *create* will default to `False` when in strict mode. Otherwise it
        ...
        :exc:`~journaltools.InvalidSectionError` is thrown.
        """
...
class Config(ConfigSection):
    ...
    that will be using when calling :meth:`~journaltools.ConfigSection.sync`.
    
    The format of the sources can be set using *format*. This can be the
    registered name of a format, such as "ini", or a :class:`~journaltools.Format`
    class or instance.
    ...
    A :class:`~journaltools.Coercer` can be set using *coercer*. If no coercer is
    ...
    This is a subclass of :class:`~journaltools.ConfigSection`.
    """
    ...
    @property
    def format(self):
        """The :class:`~journaltools.Format` to use to process sources."""
        return self._format
    ...
    def set_format(self, format):
        ...
        *format* can be the registered name of a format, such as
        "ini", or a :class:`~journaltools.Format` class or instance.
        """
...
class MetaFormat(type):
    """
    Metaclass that registers a :class:`~journaltools.Format` with the
    :class:`~journaltools.Config` class.
    """
...
class Coercer:
    ...
    def register_choice(self, type, choices):
        ...
        Values passed into :meth:`~journaltools.Coercer.adapt` or
        :meth:`~journaltools.Coercer.convert` for *type* will have to be one of the
